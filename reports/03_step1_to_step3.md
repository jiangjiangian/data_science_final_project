...
- **Run D (N≈678 + pitching, m1–m7 relocked):** in progress —
  regenerated by `run_all.py` in Colab; numbers land in
  `Results/eval/_final_metrics.json` / `results_*.csv`. Read off the
  m6 (pitching-only) and m7 (full) season-OOF-AUC vs m1 to quantify
  the pitching group's marginal contribution — the headline question.
